...
namespace InvoiceApi.Constants
{
    public static class SqlQuery
    {
        ...
        public const string SaveInvoiceDetails = @"INSERT INTO tbl_Invoice_Details (User_Id, Invoice_No,Invoice_Date,Due_Date,Business_Id,Client_Id,Notes,Terms,Tax,Discount,Total,SubTotal,Taxtype,DiscountType,Template_Name,TaxPercentage,DiscountPercentage,IsCustomDate,IsCustomDueDate,Custom_Date,Custom_DueDate,currency_code,taxlabel,DiscountLabel,TotalLabel,SubTotalLabel) OUTPUT INSERTED.Invoice_Id  VALUES(@UserId,@InvoiceNumber,@InvoiceDate,@InvoiceDueDate,@BusinessId,@ClientId,@Notes,@Terms,@Tax,@Discount,@Total,@SubTotal,@Taxtype,@DiscountType,@TemplateName,@TaxPercentage,@DiscountPercentage,@IsCustomDate,@IsCustomDueDate,@CustomDate,@CustomDueDate,@CurrencyCode,@TaxLabel,@DiscountLabel,@TotalLabel,@SubTotalLabel)";
        ...
        public const string GetInvoiceDetails = @"Select Inv.Invoice_Id as InvoiceId,Inv.Invoice_No as InvoiceNo,Inv.Invoice_date as InvoiceDate,Inv.Due_Date as InvoiceDueDate,Inv.Business_Id as BusinessId,Inv.Client_Id as ClientId,Inv.Total as InvoiceAmount,Custom_Date as CustomDate,IsCustomDate as IsCustomDate,(Select Business_Name from tbl_business_details where Business_Id=Inv.Business_Id and Isdeleted=0) as BusinessName,(Select Client_Name from tbl_client_details where Client_Id=Inv.Client_Id and Isdeleted=0) as ClientName from tbl_invoice_details Inv  where inv.user_Id=@UserId and inv.isdeleted=0";
        public const string GetInvoiceDetailByInvoiceId = @"SELECT Invoice_Id as InvoiceId,Invoice_No as InvoiceNumber,Invoice_Date as  InvoiceDate,Due_Date as InvoiceDueDate,Business_Id as BusinessId,Client_Id as ClientId,Notes,Tax,Discount,Total,Template_Name as TemplateName,Terms,SubTotal,Taxtype,DiscountType,TaxPercentage,DiscountPercentage,IsCustomDate,IsCustomDueDate,Custom_Date as CustomDate,Custom_DueDate as CustomDueDate,Currency_code as CurrencyCode,TaxLabel,DiscountLabel,TotalLabel,SubTotalLabel from tbl_invoice_details WHERE Invoice_Id=@InvoiceId and User_Id=@UserId and IsDeleted=0";
